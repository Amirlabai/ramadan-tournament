...
- **Owner/captain final join review:** if the team has an **owner** (`ownerUserId`) **or** a **claimed captain** (`isCaptain` + `userId`), new joins stay `pending` for that owner/captain; their approve **finalizes** the roster (`approved` — link/create player). Admin does **not** review those joins. If there is **neither** owner nor claimed captain, joins are created as `owner_approved` (admin queue). Queue sync promotes stuck `pending` → admin when the last reviewer disappears, and **intentionally reopens** auto-skip rows (`owner_approved` with null `ownerReviewedAt` / `adminReviewedAt`) → `pending` when an owner or claimed captain appears again. In-flight rows that already had `ownerReviewedAt` set under the old two-step flow stay on the admin queue once. Claimed players keep Profile self-edit; owner/captain may post-edit any roster player’s fields/photo (last write wins).
