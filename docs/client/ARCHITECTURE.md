...
- **Mobile traffic (verify targets):** ~41% **Safari (iOS)**; rest mainly **Chrome** + **Instagram in-app**. Spot-check Safari first on ≤768px changes (fixed chrome, stacking, safe-area, auth). See [`context.md`](../../context.md) and [`.cursor/rules/mobile-safari-instagram.mdc`](../../.cursor/rules/mobile-safari-instagram.mdc).
