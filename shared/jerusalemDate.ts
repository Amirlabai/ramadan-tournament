...
const JLM_PARTS_FORMATTER = new Intl.DateTimeFormat('en-US', {
  timeZone: 'Asia/Jerusalem',
  year: 'numeric',
  month: 'numeric',
  day: 'numeric',
...
export function getJerusalemParts(date: Date): JerusalemParts {
  const parts = JLM_PARTS_FORMATTER.formatToParts(date);
...
/** YYYY-MM-DD calendar key in Asia/Jerusalem. */
...
export function isSameJerusalemCalendarDay(
  dateIso: string,
  now: Date = new Date()
): boolean {
  return jerusalemDateKey(new Date(dateIso)) === jerusalemDateKey(now);
}
