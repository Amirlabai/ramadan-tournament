...
**Version:** 0.9  
**Status:** PRD complete — two-layer registration (§16) locked  
...
Identity, invoice gate, teams/roster/owner, workflows, **football** (matches/goals/bracket/lineup) and **points** (point_entries standings), news, comments, votes, stats snapshots, archives, banned words, photos, Redis cache.
...
Shared per `season_id`: teams, roster workflows, invoice, votes (MVP may differ for points seasons — see §6.G).
...
6. Invoice wrong-attempt rate limit in **Redis only** (lock until next calendar day).
...
| `invoice_codes` | UUID | `season_id`, `code_hash`, `assigned_user_id`, `redeemed_at`; season’s `division` implies boys **or** girls only |
...
| **2. Tournament** | Boys **or** girls season; join + invoice | `season_registrations`, requests, `invoice_codes`, `players` |
...
| `season_registrations.status` | see §16 | Tracks invoice + eligibility per `season_id` |
...
### C. Invoice (tournament layer only)
...
Join approval (owner → admin) may proceed in parallel, but **roster / full player features** require `season_registrations.status = active` (invoice redeemed).
...
| Join | Owner approve → admin final approve (can be pending while awaiting invoice) |
...
sequenceDiagram
  ...
  Note over API: active_division locked
  A->>API: Assign invoice after offline payment
  U->>API: Enter invoice on profile
  Note over API: season_registrations active
  API->>U: Full player access roster after approvals
```
...
| 3 | User | **Request to join** (or request new team) — allowed **without** invoice yet |
| 4 | Admin | In real life user pays → admin **assigns invoice code** to that user for that `season_id` |
| 5 | User | **Enters invoice** on profile (redeem) |
| 6 | System | `season_registrations.status = active`; roster/player UI fully enabled after join approvals |

### `season_registrations` status machine

| Status | Meaning |
|--------|---------|
| `none` | No tournament activity this season (or not started) |
| `join_pending` | Join/create request in flight (optional; may overlap below) |
| `awaiting_invoice` | Join path started; admin has **not** assigned code yet, or user not redeemed |
| `invoice_assigned` | Admin assigned code; user must enter it on profile |
| `active` | Invoice redeemed — eligible for roster / play this season |
| `archived` | Season ended |

**UI hints:**

- Before step 3: tournament actions on chosen side only; other side read-only or banner “לא רשום לטורניר זה”.
- Steps 3–5: show “ממתין לקוד תשלום” until redeem.
...
- **`season_registrations`** — `(user_id, season_id)` + status + `division`.
- **`invoice_codes`** — admin → user, per season.
...
| 7 | Registration | **Layer 1** website login + **Layer 2** join then invoice (§16); one `users` row; boys OR girls only |
