...
## Current Focus
- **Phase 2 (May 2026):** Tournament registration via `RegistrationService` — `season_registrations`, `invoice_codes` (admin assign + user redeem, Redis rate limit), `team_*_requests`, `active_division`, owner join review. **Flow order:** user redeems receipt on Profile first (`active`) → then join or team-creation request → admin approves (requires `active`). APIs: `/api/users/registration`, `/api/users/redeem-invoice` (3/day), `/api/users/cancel-registration-request` (10/15min), `/api/teams/creation-request`, `/:id/join-request`, admin `/api/admin/workflows`. **One pending request per user per season** (join *or* team creation). UI: Profile invoice cards + cancel, Teams join/creation actions (gated on `active`), Admin → סגל ורישום → `RegistrationWorkflowAdmin`. Team creation uses division-aware `/api/teams/creation-request` or `/api/teams-girls/creation-request` only (`POST /users/request-team` removed).
...
3. **Teams** — receipt on Profile → registration workflow (join / team creation + admin approval).
