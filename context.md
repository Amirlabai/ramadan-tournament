...
- **June 2026 — Captain scope:** Team **owners** (`ownedTeamId`) approve join requests and edit branding via `TeamOwnerSettings`. Squad **captains** (`isCaptain`) edit lineup roles via `OwnerSquadRoles`, and **claimed** captains may also edit branding (logo/metadata) and review joins — same gates as owners for those surfaces. Profile + Teams use `TournamentRoleStar` (gold / gold+blue-outline / blue). Roster add/delete and admin panel remain platform-admin only.
