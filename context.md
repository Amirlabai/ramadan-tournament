...
- **Donation popup (Jul 2026):** Boys-only bottom sheet ([`DonationPopup`](client/src/components/DonationPopup.tsx)) on Jerusalem Fri/Sat from **17:00** until end of that day (`isDonationPopupWindow` in [`shared/matchTiming.ts`](shared/matchTiming.ts)). Once per Jerusalem calendar day (`localStorage` `donationPopupShownDate`) and once per tab session (`sessionStorage`); marked when shown. Deferred until cookie notice is dismissed. Copy: volunteer/community story; CTA → `/donate.html`. Footer/legal label is `תתרמו לאתר` ([`DONATE_LABEL`](client/src/config/contactConfig.ts)) — no coffee gimmick.
