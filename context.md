...
- **Invoice `code_normalized` (threat model):** Postgres stores a normalized plaintext copy of payment reference numbers alongside `code_hash` (bcrypt) so admins can fuzzy-compare user-entered vs assigned invoices and detect duplicates. Treat `invoice_codes` as confidential at rest (DB access = payment metadata exposure). CSV backups omit `code_normalized`/`code_hash`. User-facing APIs (`/users/*`, `/auth/me`) never return `code_normalized`; admin workflow endpoints (`/admin/workflows*`) return `assignedInvoiceNumber` to authorized admins only for assign/compare UI.
