...
**Uploads (Jul 2026):** Hybrid persistence — prefer non-empty files under deploy/`server/uploads` (git via [`scripts/sync_photos.py`](scripts/sync_photos.py)); new captain logos / player photos / avatars write to `UPLOADS_DISK_PATH` (Render disk) until sync commits them. Serve order: repo (size > 0) then disk. Never commit empty placeholders under `server/uploads/`. Already-404 prod URLs cannot be recovered by sync — re-upload after disk is live, or commit the local file then deploy. `render.yaml` uses a paid plan + disk mount; ship code and disk config together.
