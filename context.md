...
- **Python (repo root):** `requirements.txt` + local `.venv/` (gitignored). First time: `python -m venv .venv` then `.\.venv\Scripts\python.exe -m pip install -r requirements.txt`. Run scripts with `.venv\Scripts\python.exe`, not system `python`. `invoice_codes` CSV exports omit `code_hash` and `code_normalized` (no plaintext payment codes).
...
## Current Focus
- **Phase 2 (May 2026):** Tournament registration via `RegistrationService` — `season_registrations`, `invoice_codes` (admin assign + user redeem, Redis rate limit), `team_*_requests`, `active_division`, owner join review. **Flow order:** admin records receipt number first (`invoice_assigned`) → user enters the **exact same** number on Profile (`active` only on match) → then join or team-creation request → admin approves join/creation (requires matched receipts + `active`; transfers unchanged). APIs: `/api/users/registration`, `/api/users/redeem-invoice` (3/day), `/api/users/cancel-registration-request` (10/15min), `/api/teams/creation-request`, `/:id/join-request`, admin `/api/admin/workflows`. **One pending request per user per season** (join *or* team creation). UI: Profile invoice cards + cancel, Teams join/creation actions (gated on matched `active`), Admin → סגל ורישום → `RegistrationWorkflowAdmin` (receipt columns on creation/join queues). Team creation uses division-aware `/api/teams/creation-request` or `/api/teams-girls/creation-request` only (`POST /users/request-team` removed).
- **Invoice `code_normalized` (threat model):** Postgres stores a normalized plaintext copy of payment reference numbers alongside `code_hash` (bcrypt) so admins can fuzzy-compare user-entered vs assigned invoices and detect duplicates. Treat `invoice_codes` as confidential at rest (DB access = payment metadata exposure). CSV backups omit `code_normalized`/`code_hash`. User-facing APIs (`/users/*`, `/auth/me`) never return `code_normalized`; admin workflow endpoints (`/admin/workflows*`) return `submittedInvoiceNumber` (read-only placeholder) and `assignedInvoiceNumber` (admin’s unredeemed assignment). When admin assigns after the user already submitted in Profile, the server compares automatically and sets `invoiceAlert` on the user’s registration if numbers differ (user contacts admin offline).
...
3. **Teams** — admin assigns receipt → user enters same number on Profile → registration workflow (join / team creation + admin approval).
