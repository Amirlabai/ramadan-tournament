...
## Current Focus
- **Phase 2 (May 2026):** Tournament registration via `RegistrationService` — `season_registrations`, `invoice_codes` (admin assign + user redeem, Redis rate limit), `team_*_requests`, `active_division`, owner join review. APIs: `/api/users/registration`, `/api/users/redeem-invoice` (3/day), `/api/users/cancel-registration-request` (10/15min), `/api/teams/creation-request`, `/:id/join-request`, admin `/api/admin/workflows`. **One pending request per user per season** (join *or* team creation); both require invoice redeem before admin final approval. UI: Profile invoice cards + cancel, Teams join/creation actions, Admin → סגל ורישום → `RegistrationWorkflowAdmin`. Team creation uses division-aware `/api/teams/creation-request` or `/api/teams-girls/creation-request` only (`POST /users/request-team` removed).
- **Invoice `code_normalized` (threat model):** Postgres stores a normalized plaintext copy of payment reference numbers alongside `code_hash` (bcrypt) so admins can fuzzy-compare user-entered vs assigned invoices and detect duplicates. Treat `invoice_codes` as confidential at rest (DB access = payment metadata exposure). CSV backups omit `code_normalized`/`code_hash`; API never returns these fields to clients.
